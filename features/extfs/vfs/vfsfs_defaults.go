...
func NewVFSFS(server *stdVFSServer) *VFSFUSEFS {
	var vfsfs VFSFUSEFS
	vfsfs.vfsServer = server
	return &vfsfs
}

var _ = (VFSFS)((*VFSFUSEFS)(nil))
...
func (fusefs *VFSFUSEFS) Mount() error {
	...
	mountPath := fusefs.vfsServer.MountPath()
	if len(mountPath) <= 0 {
		return ErrVFSFUSEFSUnavailable
	}
...
func (fusefs *VFSFUSEFS) Readdir(ctx context.Context) (fs.DirStream, syscall.Errno) {
	...
	fusefs.rw.RLock()
	hostname := fusefs.vfsServer.HostName()
	runtime := fusefs.vfsServer.runtime
	...
	if len(hostname) > 0 {
		localEntry := fuse.DirEntry{Name: hostname, Mode: fuse.S_IFDIR}
