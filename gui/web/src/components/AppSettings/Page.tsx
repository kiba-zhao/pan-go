...
import { QRCodeDownloadButton } from "../Common/QRCode";
...
const SaveButton = () => {
  ...
  const onSave = async (fields: AppSettingsFields) => {
...
const AppAddressSettings = ({ source, children }: AppAddressSettingsProps) => {
  ...
  return (
    <FormProvider {...methods}>
      <PageTopBar>{children}</PageTopBar>
...
const BroadcastAddressSettings = () => {
  ...
  return (
    <AppAddressSettings source={source}>
      <RefreshButton />
      <SaveButton />
      <NewAddressButton source={source} defaultValue="224.0.0.2:9100" />
    </AppAddressSettings>
  );
};
