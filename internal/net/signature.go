...
var ErrSignatureInvalidPrivKey = errors.New("peer.signature Error: Invalid Private Key")
var ErrSignatureInvalidPublicKey = errors.New("peer.signature Error: Invalid Public Key")
var ErrSignatureUnsupportedHash = errors.New("peer.signature Error: Unsupported Hash")
var ErrSignatureUnsupportedCurve = errors.New("peer.signature Error: Unsupported Curve")
var ErrSignatureVerifyFailed = errors.New("peer.signature Error: Verify Failed")
...
//
// The error returned is ErrSignatureInvalidPrivKey if the key is not an *ecdsa.PrivateKey,
// and ErrSignatureUnsupportedCurve if the curve is not supported.
func Sign(data []byte, key crypto.PrivateKey) ([]byte, error) {
	privKey, ok := key.(*ecdsa.PrivateKey)
	if !ok {
		return nil, ErrSignatureInvalidPrivKey
	}
...
func Verify(data, sig []byte, key any) error {
	...
	if !ok {
		return ErrSignatureInvalidPublicKey
	}
...
func hashWithECDSA(curveName string, data []byte) (hash []byte, err error) {
	...
	default:
		err = ErrSignatureUnsupportedCurve
	}
	return
}
...
func shaWithCryptoHash(hash crypto.Hash, data []byte) (hashed []byte, err error) {
	...
	case crypto.SHA512:
		...
		hashed = hash48[:]
	default:
		err = ErrSignatureUnsupportedHash
	}
	return
}
