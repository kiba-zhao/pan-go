...
const (
	PeerEventConnected = uint8(iota + 1)
	PeerEventDisconnected
)

type PeerListener interface {
	OnPeerEvent(PeerID, uint8)
}
...
type PeerNetwork interface {
	...
	AttachPeerListener(PeerListener)
	DetachPeerListener(PeerListener)
...
func (network *stdPeerNetwork) Purge(peerId PeerID) error {
	...
	for _, transport := range transports {
		...
		if err != nil {
			hasErr = true
			network.logger.Error("PeerNetwork", "Purge Error: "+err.Error())
		}
	}
...
func (network *stdPeerNetwork) AttachPeerListener(listener PeerListener) {
	network.listenersRW.Lock()
	defer network.listenersRW.Unlock()
	network.listeners = append(network.listeners, listener)
}

func (network *stdPeerNetwork) DetachPeerListener(listener PeerListener) {
	network.listenersRW.Lock()
	defer network.listenersRW.Unlock()
	network.listeners = slices.DeleteFunc(network.listeners, func(l PeerListener) bool {
		return l == listener
	})
}

func (network *stdPeerNetwork) OnPeerEvent(peerId PeerID, event uint8) {
	network.listenersRW.RLock()
	defer network.listenersRW.RUnlock()
	for _, listener := range network.listeners {
		listener.OnPeerEvent(peerId, event)
	}
}
